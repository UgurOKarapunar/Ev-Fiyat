...
# Model ve ön işleme nesnelerini yükleme
preprocess = joblib.load('preprocess.pkl')
model = load_model('model.h5')

st.title("Fiyat Tahmin Uygulaması")

# Kullanıcıdan veri girişi
st.sidebar.header("Girdi Bilgileri")

# Kategorik ve sayısal özellikler için giriş alanları
...
numerical_input2 = st.sidebar.number_input("Eğitim Kurumları Uzaklık(km)", min_value=0.0, max_value=100.0, value=0.0)
...
# Eksik sütunları içeren tüm sütunları tanımlayın
all_columns = [
    'Eşyalı/Eşyasız', 'Otopark', 'Binanın Bulunduğu Kat', 'Güvenlik',
    'Alışveriş Merkezine Uzaklık(km)', 'Eğitim Kurumları Uzaklık(km)', 'Toplu Taşımaya Uzaklık(km)',
    'Oda Sayısı', 'İlçe', 'Balkon', 'Metrekare', 'Bina Yaşı', 'Isıtma Tipi', 'Kat Sayısı'
]

# Girdi verisini DataFrame'e dönüştürme
...
# Sonuçları gösterme
st.write(f"Tahmin Edilen Fiyat: {prediction[0][0]:.2f}")
